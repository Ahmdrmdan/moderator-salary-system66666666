...
const SalaryProcessing = (() => {
  ...
  async function load(){
    ...
    if(node){const status=snapshot?.status||'draft';node.textContent=status==='paid'?'Paid':status==='approved'?'Approved':'Draft';node.className=`badge ${status==='draft'?'badge-locked':'badge-approved'}`;}
    render();
  }
