...
const SalaryProcessing = (() => {
  ...
  function paymentSummary(){
    const rows=(snapshot&&snapshot.report)||[],manual=(snapshot&&snapshot.employeeManualEntries)||{},payments=(snapshot&&snapshot.employeePayments)||{};
    const totals=aggregate(rows,manual).totals;
    const paidRows=rows.filter(row=>payments[employeeId(row)]&&payments[employeeId(row)].status==='paid');
    const paidAmount=paidRows.reduce((sum,row)=>sum+financial(row,manual[employeeId(row)]||{}).net,0);
    return { employees:rows.length, paidEmployees:paidRows.length, remainingEmployees:Math.max(rows.length-paidRows.length,0), totalNet:totals.net, paidAmount, remainingAmount:Math.max(totals.net-paidAmount,0), progress:rows.length?Math.round((paidRows.length/rows.length)*100):0 };
  }
  ...
  async function approve(){
    ...
    await commitWithAudit(batch=>{
      batch.set(ref,{version:1,period:{type:'month',monthId:c.monthId},status:'approved',approvedAt:firebase.firestore.FieldValue.serverTimestamp(),approvedBy:auth.currentUser?.email||null,report:c.rows,totals:c.totals,employeeManualEntries:{},employeePayments:{},payment:{status:'unpaid',paidAt:null,paidBy:null},salarySnapshotCreatedAt:firebase.firestore.FieldValue.serverTimestamp(),readyForPaymentAt:firebase.firestore.FieldValue.serverTimestamp(),createdAt:firebase.firestore.FieldValue.serverTimestamp(),updatedAt:firebase.firestore.FieldValue.serverTimestamp(),...workflow},{merge:false});
      batch.set(db.collection(COLLECTIONS.MONTHLY_REPORTS).doc(c.monthId),workflow,{merge:true});
      batch.set(db.collection(COLLECTIONS.MONTHLY_SUMMARIES).doc(c.monthId),workflow,{merge:true});
    },{action:'salary_processing.approved',entity:'salary_processing',operation:AuditService.OPERATION.CREATE,documentId:c.monthId,documentLabel:`Payroll ${c.monthId}`,monthId:c.monthId,severity:AuditService.SEVERITY.WARNING,details:{period:c.monthId,status:'approved',employeeCount:c.rows.length,workflowTransitions:[PayrollWorkflow.STATE.SALARY_SNAPSHOT_CREATED,PayrollWorkflow.STATE.READY_FOR_PAYMENT]}});
  ...
  function init(){if(initialized)return;initialized=true;$('salaryExecutiveSummary').addEventListener('click',e=>{const b=e.target.closest('[data-salary-drill]');if(b)drillKpi(b.dataset.salaryDrill);});$('salaryDepartmentSummary').addEventListener('click',e=>{const b=e.target.closest('[data-salary-department]');if(b)drillDepartment(b.dataset.salaryDepartment);});$('salaryRankingBody').addEventListener('click',e=>{const row=e.target.closest('[data-details]');if(row)details(row.dataset.details);});['salaryRankingMetric','salaryRankingDirection'].forEach(id=>$(id).addEventListener('change',()=>{if(snapshot)renderRanking(snapshot.report||[],snapshot.employeeManualEntries||{});}));const workflowRoot=$('reportWorkspaceManager');if(workflowRoot)workflowRoot.addEventListener('click',e=>{const approveButton=e.target.closest('#salarySnapshotApproveBtn');if(approveButton){approve().catch(err=>Toast.show('تعذر الاعتماد: '+err.message,'error'));return;}const payAll=e.target.closest('#salaryMarkAllPaidBtn');if(payAll){pay((snapshot?.report||[]).map(employeeId)).catch(err=>Toast.show('تعذر تسجيل الدفع: '+err.message,'error'));return;}const payOne=e.target.closest('[data-pay]');if(payOne)pay([payOne.dataset.pay]).catch(err=>Toast.show('تعذر تسجيل الدفع: '+err.message,'error'));});$('salarySnapshotRows').addEventListener('click',e=>{const b=e.target.closest('[data-adjust]');if(b)adjust(b.dataset.adjust).catch(err=>Toast.show('تعذر التعديل: '+err.message,'error'));const d=e.target.closest('[data-details]');if(d)details(d.dataset.details);});$('salaryDrawerCloseBtn').addEventListener('click',()=>$('salaryEmployeeDrawer').classList.remove('open'));$('salaryDrawerTabs').addEventListener('click',e=>{const b=e.target.closest('[data-salary-tab]');if(b&&drawerId)details(drawerId,b.dataset.salaryTab);});$('salarySnapshotExcelBtn').addEventListener('click',()=>{try{exportSnapshot();}catch(err){Toast.show('تعذر التصدير: '+err.message,'error');}});$('salarySnapshotPrintBtn').addEventListener('click',()=>{try{Permissions.require('salary_processing.export');window.print();}catch(err){Toast.show('تعذر الطباعة: '+err.message,'error');}});load().catch(()=>{});}
